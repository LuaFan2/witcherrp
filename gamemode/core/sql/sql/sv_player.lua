...
function rp.sql.CreateCharacterTable()
	sql.Begin()
	q = sql.Query('CREATE TABLE IF NOT EXISTS witcher_characters(SteamID TEXT UNIQUE, name TEXT, model TEXT, money INTEGER, time INTEGER)')
	sql.Commit()
	return q
end
...
function rp.sql.CreateCharacter(ply, name, model)
	...
	if not exists then
		q = sql.Query("INSERT INTO witcher_characters( SteamID, name, model, money, time ) VALUES( '"..ply:SteamID().."', '"..name.."', '"..model.."', '"..rp.Config.StartMoney.."', 0)")
	end
	sql.Commit()
	return q
end
